...
ssh -i ~/.ssh/otrera ubuntu@$ip_address "aws --region eu-central-1 s3 cp s3://mazerty-argos/cache.tar.gz - | gunzip | docker load"
ssh -i ~/.ssh/otrera ubuntu@$ip_address -t "sh -c 'cd ~/workspace/kantoku/docker/ && ./build.sh'"
ssh -i ~/.ssh/otrera ubuntu@$ip_address "docker save $(docker image ls -qa) | gzip | aws --region eu-central-1 s3 cp - s3://mazerty-argos/cache.tar.gz"
